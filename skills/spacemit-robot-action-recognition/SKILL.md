...
metadata:
  ...
  sdk:
    ...
    primary_docs:
      ...
      - middleware/ros2/perception/action_recognition/test.yaml
...
- 依赖 `vision` 包（对应组件路径 `components/model_zoo/vision`）、`body_pose` 包和 STGCN 模型文件
...
3. 在 `middleware/ros2/perception` 下 `colcon build --packages-select body_pose action_recognition`（与 CI 一致）。
...
```bash
cd "$SPACEMIT_SDK_ROOT/middleware/ros2/perception"
bash action_recognition/tests/test_functional.sh
bash action_recognition/tests/test_invalid_input.sh
bash action_recognition/tests/test_performance.sh   # 无模型时会 SKIP
```
...
```bash
cd "$SPACEMIT_SDK_ROOT"
scripts/test/robot-test run middleware/ros2/perception/action_recognition --scope scheduled --category performance
```
...
| 手跑 PR 测试 | `cd "$SPACEMIT_SDK_ROOT/middleware/ros2/perception" && bash action_recognition/tests/test_functional.sh` |
