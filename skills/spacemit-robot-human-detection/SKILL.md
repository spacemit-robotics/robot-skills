...
metadata:
  ...
  sdk:
    ...
    primary_docs:
      ...
      - middleware/ros2/perception/human_detection/test.yaml
...
```bash
cd "$SPACEMIT_SDK_ROOT/middleware/ros2/perception"
bash human_detection/tests/test_functional.sh
bash human_detection/tests/test_invalid_input.sh
bash human_detection/tests/test_performance.sh
```
...
```bash
cd "$SPACEMIT_SDK_ROOT"
scripts/test/robot-test run middleware/ros2/perception/human_detection --scope scheduled --category performance
```
...
| 手跑 PR 测试 | `cd "$SPACEMIT_SDK_ROOT/middleware/ros2/perception" && bash human_detection/tests/test_functional.sh` |
