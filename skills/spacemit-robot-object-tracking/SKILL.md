...
metadata:
  ...
  sdk:
    ...
    primary_docs:
      ...
      - middleware/ros2/perception/object_tracking/test.yaml
...
```bash
cd "$SPACEMIT_SDK_ROOT/middleware/ros2/perception"
bash object_tracking/tests/test_functional.sh
bash object_tracking/tests/test_invalid_input.sh
bash object_tracking/tests/test_performance.sh
```
...
```bash
cd "$SPACEMIT_SDK_ROOT"
scripts/test/robot-test run middleware/ros2/perception/object_tracking --scope scheduled --category performance
```

说明：
- object_tracking 的 scheduled performance 当前仅测初始化（`init_ms`），不测 first-output。
- 脚本会分别跑 ByteTrack 与 OC-SORT，并在同一产物中追加分项指标（如 `init_ms_bytetrack`、`init_ms_ocsort`）。
- 默认指标文件：`middleware/ros2/perception/perception_perf_object_tracking.txt`（由 `test.yaml` 的 artifacts 收集）。
...
| 手跑 PR 测试 | `cd "$SPACEMIT_SDK_ROOT/middleware/ros2/perception" && bash object_tracking/tests/test_functional.sh` |
